  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        localStorage.setItem("userType", data.user.permissions === 2 ? "worker" : "user")
        navigate(data.user.permissions === 2 ? "/worker-dashboard" : "/dashboard")
      } else {
            ...
            <CCardGroup>
              ...
              <CCard className="py-5 bg-distortion" style={{ width: "44%" }}>
                <CCardBody className="text-center">
                  <div>
                    <h2>Regístrate</h2>
                    <p>Únete a nuestra plataforma de clínica veterinaria como cliente, empleado o administrador.</p>
                    <CButton
                      color="primary"
                      className="mt-3"
                      active
